...
export default class Controller {
    constructor(model, view) {
        ...
        this.view
            ...
            .on('previewPost', this.handlePreview.bind(this));
    ...
    startAutoUpdate() {
        ...
        const update = async () => {
            console.log('🔄 Цикл обновления запущен, время:', new Date().toLocaleTimeString());
        ...
        this.updateTimer = setTimeout(update, this.UPDATE_INTERVAL);
        console.log('✅ Первая проверка запланирована через', this.UPDATE_INTERVAL, 'мс');
    }
    ...
    handlePreview(postId) {
        console.log('👁️ Предпросмотр поста:', postId);

        // 1. Найти пост во всех фидах
        // Так как у нас нет прямого доступа к постам в контроллере,
        // нужно либо получить все посты через model.postsManager
        const allPosts = this.model.postsManager.getAllPosts();
        ...
        // 2. Отметить как прочитанный
        this.model.postsManager.markAsRead(Number(postId));

        // 3. Показать в модальном окне
        this.view.showPostPreview(post);

        // 4. Обновить отображение списка постов
        // Нужно перерендерить текущий фид, чтобы изменился класс fw-bold → fw-normal
        const currentFeed = this.model.feedsManager
            .getFeeds()
            .find((feed) => feed.url === post.feedUrl);
        if (currentFeed) {
            const feedPosts = this.model.postsManager.getPostsByFeedUrl(post.feedUrl);
            this.view.displayPosts(feedPosts, currentFeed.title);
        }
    }
